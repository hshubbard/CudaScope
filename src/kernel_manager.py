...
BASE_DIR       = SRC_DIR.parent                 # repo root (cwd for all subprocesses)
KERNELS_DIR    = BASE_DIR / "kernels"           # kernels/
REGISTRY_FILE  = KERNELS_DIR / "user_kernels.json"
USER_CUH       = KERNELS_DIR / "user_kernels.cuh"
USER_CU        = KERNELS_DIR / "user_kernels.cu"
USER_BENCH_CU  = KERNELS_DIR / "user_benchmark.cu"
...
def run_pipeline(
    ...
    if not skip_build:
        build_sources = []
        if include_builtins:
            build_sources.append(str(KERNELS_DIR / "kernels.cu"))
        build_sources += [str(KERNELS_DIR / "user_kernels.cu"),
                          str(KERNELS_DIR / "user_benchmark.cu")]
        steps.append(("Build", [
            "nvcc", "-O3", "-std=c++17",
            "--allow-unsupported-compiler",
            f"-gencode", f"arch=compute_{sm},code=sm_{sm}",
            *build_sources,
            "-o", str(BASE_DIR / ("benchmark_user.exe" if os.name == "nt" else "benchmark_user")),
        ]))

    if not skip_bench:
        bench_bin = "benchmark_user.exe" if os.name == "nt" else "benchmark_user"
        steps.append(("Benchmark", [str(BASE_DIR / bench_bin)]))
    ...
    if not skip_ptx:
        ...
        ptx_sources = []
        if include_builtins:
            ptx_sources.append(str(KERNELS_DIR / "kernels.cu"))
        # Only include user_kernels.cu if it has actual kernel definitions
        registry = load_registry()
        if registry:
            ptx_sources.append(str(KERNELS_DIR / "user_kernels.cu"))
    ...
    if not skip_analyze:
        ...
        if not _has_kernels:
            ...
            for pass_script, pass_label, json_out in _pass_specs:
                ...
                try:
                    proc = subprocess.Popen(
                        [py, str(SRC_DIR / pass_script), "--json", json_out] + _ptx_file_args,
                        stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                        text=True, cwd=str(BASE_DIR),
                    )
                    for line in proc.stdout:
            ...
            try:
                proc = subprocess.Popen(
                    [py, str(SRC_DIR / "summary_report.py"),
...
def _run_ptx_split_and_parse(py: str, log: Callable[[str], None]):
    ...
    if pairs:
        proc = subprocess.Popen(
            [py, str(SRC_DIR / "ptx_parser.py")] + pairs,
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
            text=True, cwd=str(BASE_DIR),
        )
        for line in proc.stdout:
